...
void Topology::read_gromacs_top(std::string fname){
    ...
    while(getline(f,line)){
        ...
        if(mode=="defaults"){
            ...
            switch(comb){
                case 1:
                    comb_rule = COMB_RULE_GROMOS;
                    break;
                case 2:
                    comb_rule = COMB_RULE_AMBER;
                    break;
                case 3:
                    comb_rule = COMB_RULE_OPLS;
                    break;
                default:
    ...
    for(map<string,int>::iterator it = molecules.begin();it!=molecules.end();it++){
        ...
        for(int m=0;m<n;++m){
            ...
            for(int i=0; i<moleculetypes[mtype].atoms.size(); ++i){
                ...
                system->atoms[ind].mass = moleculetypes[mtype].atoms[i].mass;
                charge(ind) = moleculetypes[mtype].atoms[i].charge;
                type(ind) = moleculetypes[mtype].atoms[i].type;
            }
            // Assign 1-4 pairs
            ...
            for(int i=0; i<moleculetypes[mtype].bonds.size(); ++i){
                ...
                bonds.push_back(Vector2i(at1,at2));
            }
            // Assign angles
    ...
    for(int i=0;i<Natoms;++i){
        sigma(i) = atom_types[type(i)].sigma;
        epsilon(i) = atom_types[type(i)].epsilon;
    }
