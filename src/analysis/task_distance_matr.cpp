...
Task_distance_matr::Task_distance_matr(Trajectory_processor *engine, Options_tree *opt):
...
void Task_distance_matr::pre_process(){
    system.frame_dup(0);
...
bool Task_distance_matr::process_frame(const Frame_info &info){
    ...
    if(dist>0){
        ...
        Grid_searcher(dist,sel[0],bon);
        for(i=0;i<bon.size();++i){
            if(abs(bon[i](0)-bon[i](1))>4){
                r = (sel[0].XYZ(bon[i](0))-sel[0].XYZ(bon[i](1))).norm();
...
void Task_distance_matr::post_process(const Frame_info &info){
    string fname;
    ofstream f;
...
void Task_distance_matr::print_help(){
    cout << "Task distance_matr:\n"
            "-----------------\n"
            "Computes the matrix of mean distances and the matrix of distance dispersions.\n"
         << endl;
}
